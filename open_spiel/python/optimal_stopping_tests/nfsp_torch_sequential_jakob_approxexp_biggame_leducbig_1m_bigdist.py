...
def main(unused_argv):
    ...
    params["obs"] = " ".join(list(map(lambda x: str(x),obs)))
    params["obs_dist"] = " ".join(list(map(lambda x: str(x),obs_dist)))
    params["obs_dist_intrusion"] = " ".join(list(map(lambda x: str(x),obs_dist_intrusion)))
    ...
    network_parameters = {'batch_size': 256, 'hidden_layers_sizes': [128,128,128], 'memory_rl': 600000, 'memory_sl': 30000000, 'rl_learning_rate': 0.1, 'sl_learning_rate': 0.01}
    ...
    avg_attacker_br_R_array = []
    avg_defender_br_R_array = []
    approx_exp_NFSP_array = []
    ...
    for ep in range(num_train_episodes):
        if (ep + 1) % eval_every == 0 and ep+1 > 9000:
            ...
            approx_exp_NFSP = OptimalStoppingGameUtil.approx_exploitability(agents,env)
            ...
            approx_exp, avg_attacker_br_R, avg_defender_br_R = approx_exp_obj.approx_exploitability()
            ...
            ep_array.append(ep)
            avg_attacker_br_R_array.append(avg_attacker_br_R)
            avg_defender_br_R_array.append(avg_defender_br_R)
            approx_exp_NFSP_array.append(approx_exp_NFSP)
    ...
    evaluate_agents(agents, expl_array, approx_expl_array, game_value_array, game_value_array_random, game_value_array_heur, \
        avg_attacker_br_R_array,  avg_defender_br_R_array , approx_exp_NFSP_array)
...
def evaluate_agents(agents, expl_array, approx_expl_array, game_value_array, game_value_array_random, game_value_array_heur, \
    ...
    save_name = "Exploit_new_code_base_with_good_params_leduc_2m_bigdist" + str(experiment_no)
    ...
    if not os.path.isfile(save_name+".csv"):
        ...
        df["approx_exp_NFSP_array"] = approx_exp_NFSP_array
        ...
        df["avg_attacker_br_R_array"] = avg_attacker_br_R_array
        df["avg_defender_br_R_array"] = avg_defender_br_R_array
